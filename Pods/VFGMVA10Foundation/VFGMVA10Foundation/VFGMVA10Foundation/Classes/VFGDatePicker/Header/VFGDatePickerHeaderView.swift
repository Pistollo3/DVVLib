...
class VFGDatePickerHeaderView: UIView {
    ...
    lazy var previousMonthButton: VFGButton = {
        ...
        button.setBackgroundImage(VFGImage(named: "icChevronLeft"), for: .normal)
    ...
    lazy var nextMonthButton: VFGButton = {
        let button = VFGButton()
        button.translatesAutoresizingMaskIntoConstraints = false
        button.setBackgroundImage(VFGImage(named: "icChevronRight"), for: .normal)
        button.contentEdgeInsets = UIEdgeInsets(top: 5, left: 5, bottom: 5, right: 5)
    ...
    func buildDayOfWeek() {
        ...
        (firstWeekDay...(7 + firstWeekDay))
            ...
            .forEach { dayNumber in
                ...
                dayLabel.text = datePicker.dataSource?.dayOfWeekLetter(for: dayNumber)
