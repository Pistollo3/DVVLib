...
public class VFGDatePickerCollectionViewCell: UICollectionViewCell {
    ...
    public override func layoutSubviews() {
        ...
        let defaultCellSize: CGFloat = 45
        let compactCellSize: CGFloat = 60
        let size = traitCollection.horizontalSizeClass == .compact
            ? min(min(frame.width, frame.height) - 10, compactCellSize)
            : defaultCellSize
        ...
        NSLayoutConstraint.activate([
            ...
            todayBackgroundView.widthAnchor.constraint(equalToConstant: size),
            todayBackgroundView.heightAnchor.constraint(equalTo: todayBackgroundView.widthAnchor)
        ])

        selectionBackgroundView.layer.cornerRadius = size / 2
        todayBackgroundView.layer.cornerRadius = size / 2
    }
